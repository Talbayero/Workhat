...
jest.mock("@/lib/email-connector/encryption", () => ({
  encryptSecret: jest.fn((value: string) => `enc:${value}`),
}));

jest.mock("@/lib/email-connector/google", () => ({
...
import { importRecentGmailInbox, markGmailSyncSuccess } from "@/lib/email-connector/gmail-importer";
import { exchangeGmailCode, fetchGmailProfile } from "@/lib/email-connector/google";
