-- ─────────────────────────────────────────────────────────────────────────────
-- 0024_harden_remaining_function_search_paths.sql
--
-- Hardens the two SECURITY DEFINER functions that were not covered by
-- migration 0023:
--
--   • bootstrap_user_organization  (first defined in 0013)
--   • increment_company_open_conversations  (first defined in 0013)
--
-- Both already use fully-qualified public.tablename references throughout,
-- so switching to SET search_path = '' requires no body changes.
--
...
-- ── increment_company_open_conversations ──────────────────────────────────────
