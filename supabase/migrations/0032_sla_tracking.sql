-- Migration 0032 — SLA tracking and queue-health support
--
-- SLA policy is configured per org. Conversation rows keep a deterministic
-- snapshot so queue views can filter quickly and operators can see why a
-- thread is at risk or breached without recomputing message history on read.
...
comment on table public.org_sla_policies is
  'Per-organization SLA policy for deterministic first-response and next-response tracking.';

comment on column public.conversations.sla_status is
  'Current SLA snapshot: not_applicable, ok, at_risk, or breached.';

comment on column public.conversations.sla_due_at is
  'Current active SLA deadline, derived from message history and org_sla_policies.';
